...
func main() {
	...
	// arg := "/home/ubuntu/ms/fabric-samples/asset-transfer-basic/chaincode-go/chaincode/smartcontract.go"
	// arg := "/home/ubuntu/ms/fabric-samples/chaincode/fabcar/go/fabcar.go"
	// arg := "/home/ubuntu/ms/fabric-samples/asset-transfer-basic/experiment/chaincode/smartcontract.go"
	// arg := "./testdata/2+/ewallet.go"
	// runfile = 1
...
func analyzeFiles(logger *log.Logger, filenames ...string) {
	...
	for _, p := range ps {
		if p.Validity {
			...
			if p.Function == "Data Safe" {
				fmt.Println("## Comment ", p.Comment)
			} else {
