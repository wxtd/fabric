...
type Problem struct {
	...
	Comment          string // Comment about some other problems
	Validity         bool
}
...
func (si *storedInfo) init(funcName string) {
	...
	//si.targetVar[funcName]["PutPrivateData"] = make(map[token.Pos]ast.Expr)
	//si.targetVar[funcName]["GetPrivateData"] = make(map[token.Pos]ast.Expr)
...
//
func createProblem(si *storedInfo, varName, targetFuncName, category, comment string, pos token.Pos, origPos token.Pos) Problem {
...
func (f *file) checkImports(si *storedInfo) {
	...
	for _, i := range f.f.Imports {
		...
		case "github.com/hyperledger/fabric/core/chaincode/shim", "github.com/hyperledger/fabric-chaincode-go/shim":
			si.flagNonDet["RangeQuery"] = true
			si.flagNonDet["CrossChan"] = true
			si.fabricVersion = "fabric2-"
		case "github.com/hyperledger/fabric-contract-api-go/contractapi":
			si.fabricVersion = "fabric2+"
		}
...
// for checking interface satisfaction, we use directory to check the methods' names and the signatures
					...
					if typespec, ok := spec.(*ast.TypeSpec); ok {
						if st, ok := typespec.Type.(*ast.StructType); ok {
							if st.Fields.List != nil {
								...
								stMap[typespec.Name.Name]["Init"] = false   // no problem
								stMap[typespec.Name.Name]["Invoke"] = false // no problem
		...
		if fd, ok := i.(*ast.FuncDecl); ok {
			...
			if fd.Recv != nil {
				if fd.Name.Name == "Init" || fd.Name.Name == "Invoke" {
					var recvname string // interface name
					...
					if _, ok := stMap[recvname]; ok {
						...
						for _, pkgName := range []string{"github.com/hyperledger/fabric/core/chaincode/shim", "github.com/hyperledger/fabric-chaincode-go/shim"} {
							...
							if pflag {
								...
								if pkgName == "github.com/hyperledger/fabric/core/chaincode/shim" {
									pkgName = si.importMap["github.com/hyperledger/fabric/protos/peer"]
								} else {
									pkgName = si.importMap["github.com/hyperledger/fabric-protos-go/peer"]
								}
...
func (f *file) checkExternalLibrary(si *storedInfo, fullpath, abbrev string, pos token.Pos) {
	...
	if !strings.Contains(fullpath, "github.com/hyperledger") {
		if strings.Contains(fullpath, "bcrypt") || strings.Contains(fullpath, "crypto") {
			// package with crypto
			si.flagDataSafe["Crypto"] = true
			return
		}
...
func checkShim(si *storedInfo, id *ast.Ident) bool {
	if field, ok := id.Obj.Decl.(*ast.Field); ok {
		if fieldType, ok := field.Type.(*ast.SelectorExpr); ok {
			...
			if (x == si.importMap["github.com/hyperledger/fabric/core/chaincode/shim"] || x == si.importMap["github.com/hyperledger/fabric-chaincode-go/shim"]) && sel == "ChaincodeStubInterface" {
				return true
			}
		}
	}
...
func checkcontractapi(si *storedInfo, id *ast.Ident) bool {
	if field, ok := id.Obj.Decl.(*ast.Field); ok {
		if fieldType, ok := field.Type.(*ast.SelectorExpr); ok {
			x := fieldType.X.(*ast.Ident).Name
			sel := fieldType.Sel.Name
			if x == si.importMap["github.com/hyperledger/fabric-contract-api-go/contractapi"] && sel == "TransactionContextInterface" {
				return true
			}
		}
	}
	...
	case *ast.CallExpr:
		if se, ok := t.Fun.(*ast.SelectorExpr); ok {
			if se.Sel.Name == "PutState" {
				if sei, ok := se.X.(*ast.Ident); (ok && checkShim(si, sei)) || (!ok && checkcontractapi(si, se.X.(*ast.CallExpr).Fun.(*ast.SelectorExpr).X.(*ast.Ident))) {
					// shim
			...
			} else if strings.Contains(se.Sel.Name, "PutPrivateData") || strings.Contains(se.Sel.Name, "GetPrivateData") {
				si.flagDataSafe["PrivateData"] = true
			}
		}
	case *ast.ForStmt:
...
func (f *file) siInit() *storedInfo {
	return &storedInfo{
		...
		//"Crypto": false, "PrivateData": false},
		flagDataSafe: map[string]bool{"Crypto": false, "PrivateData": false},
	}
}

func (f *file) analyze() {
	//fmt.Println("Starting analyze")
	si := f.siInit()
	//fmt.Println("init done")
	f.checkImports(si)
	//fmt.Println("checkImports done")
	f.checkGlobalVar(si)
	//fmt.Println("checkGlobalVar done")
	if si.fabricVersion == "fabric2-" {
		f.checkFieldDeclaration(si)
	}
	//fmt.Println("checkFieldDeclaration done")
	f.storeInfo(si)
	//fmt.Println("storeInfo done")
	f.detectProblems(si)
	//fmt.Println("detectProblems done")
	f.checkDataSafe(si)
	//fmt.Println("checkDataSafe done")
	isValid(si)
	//fmt.Println("isValid done")
...
// check Unencrypted Sensitive Data && Unused Privacy Data Mechanism
func (f *file) checkDataSafe(si *storedInfo) {
	if !si.flagDataSafe["Crypto"] {
		if _, ok := si.isValidProblem["Data Safe"]; !ok {
			si.isValidProblem["Data Safe"] = make(map[string]map[string]bool)
		}
		comment := "If there is a potential risk about Unencrypted Sensitive Data in the chaincodes, " +
			"\nthe plaintext of sensitive transaction data is stored on the ledger, which may cause the leakage of transaction data." +
			"For example, you can import some crypto package to do this. "
		si.problems = append(si.problems, createProblem(si, "", "Data Safe", "Unencrypted Sensitive Data", comment, 0, 0))
	}

	if !si.flagDataSafe["PrivateData"] {
		if _, ok := si.isValidProblem["Data Safe"]; !ok {
			si.isValidProblem["Data Safe"] = make(map[string]map[string]bool)
		}
		comment := "For the protection of sensitive data, Hyperledger Fabric also provides a Private Data Mechanism to enhance the security of transaction data.\n" +
			"If there is Unused Privacy Data Mechanism in the chaincodes, the security of transaction data may be weakened.\n" +
			"For example, you can use GetPrivateData & PutPrivateData API. "
		si.problems = append(si.problems, createProblem(si, "", "Data Safe", "Unused Privacy Data Mechanism", comment, 0, 0))
	}
}
