...
set -x
peer channel fetch 0 $BLOCKFILE -o orderer0.example.com:7050 --ordererTLSHostnameOverride orderer0.example.com -c $CHANNEL_NAME --tls --cafile "$ORDERER0_CA" >&log.txt
res=$?
{ set +x; } 2>/dev/null
cat log.txt
